...
KIFlyTo::KIFlyTo(DronePosition checkpointP, double stayTime, double maxControlFactorP, double initialReachedDistP,
                 double stayWithinDistP)
{
  ...
  char buf[200];
  snprintf(buf, 200, "goto %.2f %.2f %.2f %.2f", checkpointP.pos[0], checkpointP.pos[1], checkpointP.pos[2],
           checkpointP.yaw);
  command = buf;
}
...
bool KIFlyTo::update(const tum_ardrone::filter_stateConstPtr statePtr)
{
  ...
  // get target dist:
  TooN::Vector < 3 > diffs = TooN::makeVector(statePtr->x - checkpoint.pos[0], statePtr->y - checkpoint.pos[1],
