...
private:
  enum
  {
    NONE, STARTED, WAIT_FOR_FIRST, TOOK_FIRST, WAIT_FOR_SECOND, DONE
  } stage;
  int stageStarted;
  bool nextUp;
  bool resetMap;
...
public:
  KIAutoInit(bool resetMap = true, int imoveTimeMS = 500, int iwaitTimeMS = 800, int reachHeightMS = 6000,
             float controlMult = 1.0, bool takeoff = true);
  ~KIAutoInit(void);
