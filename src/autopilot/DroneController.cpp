...
ControlCommand DroneController::update(tum_ardrone::filter_stateConstPtr state)
{
  ...
  ptamIsGood = state->ptamState == state->PTAM_BEST || state->ptamState == state->PTAM_GOOD
      || state->ptamState == state->PTAM_TOOKKF;
  scaleAccuracy = state->scaleAccuracy;

  // calculate (new) errors.
  TooN::Vector < 4 > new_err = TooN::makeVector(target.pos[0] - pose[0], target.pos[1] - pose[1],
                                                target.pos[2] - pose[2], target.yaw - yaw);
...
void DroneController::setTarget(DronePosition newTarget)
{
  ...
  char buf[200];
  snprintf(buf, 200, "New Target: xyz = %.3f, %.3f, %.3f,  yaw=%.3f", target.pos[0], target.pos[1], target.pos[2],
           target.yaw);
  ROS_INFO("%s", buf);
...
void DroneController::calcControl(TooN::Vector<4> new_err, TooN::Vector<4> new_velocity, double yaw, double pitch,
                                  double roll)
{
  ...
  double deltaT = getMS() / 1000.0 - lastTimeStamp;
  ...
  double clippedZForce = std::max((-9.8 * droneMassInKilos) * 0.8,
                                  (2 * droneMassInKilos * (lastSentControl.gaz + new_velocity[2])) / deltaT);
  ...
  logInfo = TooN::makeVector(springForceRoll, springForcePitch, springForceGaz, springForceYaw, dampingForceRoll,
                             dampingForcePitch, dampingForceGaz, dampingForceYaw, deltaT, deltaYaw, deltaGaz,
                             new_velocity[0], new_velocity[1], new_velocity[2], new_velocity[3], new_err[2], new_err[3],
                             clippedZForce, lastSentControl.roll, lastSentControl.pitch, lastSentControl.gaz,
                             lastSentControl.yaw, appliedEngineForce, yawRad, pitchRad, rollRad, target.pos[0],
