    ...
    mCamera(TCam)
{
  ...
  GV3::Register(mgvdUpdateConvergenceLimit, "Bundle.UpdateSquaredConvergenceLimit", BUNDLE_UPDATE_SQUARED_CONV_LIMIT,
                SILENT);
  ...
  bool Bundle::Do_LM_Step(bool *pbAbortSignal)
  {
    ...
    while (dNewError > dCurrentError && !mbConverged && !mbHitMaxIterations && !*pbAbortSignal)
    {
      ...
      for (unsigned int i = 0; i < mvPoints.size(); i++)
      {
        ...
        for (vector<OffDiagScriptEntry>::iterator it = p.vOffDiagonalScript.begin(); it != p.vOffDiagonalScript.end();
            it++)
        {
      ...
      cout << setprecision(1) << "L" << mdLambda << setprecision(3) << "\tOld " << dCurrentError << "  New "
          << dNewError << "  Diff " << dCurrentError - dNewError << "\t";
