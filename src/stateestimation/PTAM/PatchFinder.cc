...
void PatchFinder::MakeTemplateCoarse(MapPoint &p, SE3<> se3CFromW, Matrix<2> &m2CamDerivs)
{
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint &p)
{
  ...
  if (bNeedToRefreshTemplate)
  {
    ...
    nOutside = CVD::transform(p.pPatchSourceKF->aLevels[p.nSourceLevel].im, mimTemplate, m2, vec(p.irCenter),
                              vec(mirCenter));
  ...
  for (ir.y = 1; ir.y < mnPatchSize - 1; ir.y++)
  {
    ...
    for (ir.x = 1; ir.x < mnPatchSize - 1; ir.x++)
    {
      float fPixel =   // Calc target interpolated pixel
          fMixTL * pTopLeftPixel[0] + fMixTR * pTopLeftPixel[1] + fMixBL * pTopLeftPixel[nRowOffset]
