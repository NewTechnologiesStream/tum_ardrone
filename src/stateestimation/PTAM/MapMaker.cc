...
bool MapMaker::InitFromStereo(KeyFrame &kF, KeyFrame &kS, vector<pair<ImageRef, ImageRef> > &vTrailMatches,
                              SE3<> &se3TrackerPose, SE3<> KFZeroDesiredCamFromWorld, SE3<> KFOneDesiredCamFromWorld)
{
  ...
  double dTransIMUMagn = sqrt(
      (double)((KFZeroDesiredCamFromWorld.get_translation() - KFOneDesiredCamFromWorld.get_translation())
  ...
  *pkSecond = kS;
  ...
  printf("resulting PTAM Translation: (%.4f, %.4f, %.4f), IMU translation: (%.4f, %.4f, %.4f)", ptamTrans[0],
         ptamTrans[1], ptamTrans[2], imuTrans[0], imuTrans[1], imuTrans[2]);
...
bool MapMaker::AddPointEpipolar(KeyFrame &kSrc, KeyFrame &kTarget, int nLevel, int nCandidate)
{
  ...
  v3New = kTarget.se3CfromW.inverse()
      * ReprojectPoint(kSrc.se3CfromW * kTarget.se3CfromW.inverse(), mCamera.UnProject(v2RootPos),
...
void MapMaker::BundleAdjustRecent()
{
  ...
  set<MapPoint*> sMapPoints;
  for (set<KeyFrame*>::iterator iter = sAdjustSet.begin(); iter != sAdjustSet.end(); iter++)
  {
...
void MapMaker::BundleAdjust(set<KeyFrame*> sAdjustSet, set<KeyFrame*> sFixedSet, set<MapPoint*> sMapPoints,
                            bool bRecent)
{
  ...
  if (nAccepted > 0)
  {
    ...
    for (map<MapPoint*, int>::iterator itr = mPoint_BundleID.begin(); itr != mPoint_BundleID.end(); itr++)
      itr->first->v3WorldPos = b.GetPoint(itr->second);

    for (map<KeyFrame*, int>::iterator itr = mView_BundleID.begin(); itr != mView_BundleID.end(); itr++)
...
SE3<> MapMaker::CalcPlaneAligner()
{
  ...
  for (int i = 0; i < nRansacs; i++)
  {
    ...
    Vector < 3 > v3Mean = 0.33333333
        * (mMap.vpPoints[nA]->v3WorldPos + mMap.vpPoints[nB]->v3WorldPos + mMap.vpPoints[nC]->v3WorldPos);
