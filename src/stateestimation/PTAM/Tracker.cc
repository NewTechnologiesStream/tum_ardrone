...
Tracker::Tracker(ImageRef irVideoSize, const ATANCamera &c, Map &m, MapMaker &mm) :
    mMap(m), mMapMaker(mm), mCamera(c), mRelocaliser(mMap, mCamera), mirSize(irVideoSize)
{
...
void Tracker::TrackForInitialMap()
{
  ...
  if (mnInitialStage == TRAIL_TRACKING_STARTED)
  {
    ...
    if (mbUserPressedSpacebar)
    {
      ...
      bool succ = mMapMaker.InitFromStereo(mFirstKF, mCurrentKF, vMatches, mse3CamFromWorld, KFZeroDesiredCamFromWorld,
