...
void HomographyInit::DecomposeHomography()
{
  ...
  {
    mvDecompositions[i].se3SecondFromFirst.get_rotation() = s * U * mvDecompositions[i].m3Rp * V.T();
    mvDecompositions[i].se3SecondFromFirst.get_translation() = U * mvDecompositions[i].v3Tp;
  }
}
...
void HomographyInit::ChooseBestDecomposition()
{
  ...
  for (unsigned int i = 0; i < mvDecompositions.size(); i++)
  {
    ...
    for (unsigned int m = 0; m < mvHomographyInliers.size(); m++)
    {
      ...
      double dVisibilityTest = (mm3BestHomography[2][0] * v2[0] + mm3BestHomography[2][1] * v2[1]
