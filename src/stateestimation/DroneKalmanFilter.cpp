...
{
  // add a constant of 20ms // 40ms to account for delay due to ros.
  // very, very rough approximation.
  navPing += 20;
  vidPing += 40;

  int new_delayXYZ = base_delayXYZ;
  int new_delayVideo = base_delayVideo + vidPing / 2 - navPing / 2;
  ...
  std::cout << "new delasXYZ: " << delayXYZ << ", delayVideo: " << delayVideo << ", delayControl: " << delayControl
      << std::endl;
}
...
void DroneKalmanFilter::reset()
{
  ...
  baselineZ_Filter = baselineZ_IMU = -DBL_MAX;
...
void DroneKalmanFilter::predictInternal(geometry_msgs::Twist activeControlInfo, int timeSpanMicros,
                                        bool useControlGains)
{
  ...
  bool controlValid =
      !(activeControlInfo.linear.z > 1.01 || activeControlInfo.linear.z < -1.01 || activeControlInfo.linear.x > 1.01
          || activeControlInfo.linear.x < -1.01 || activeControlInfo.linear.y > 1.01
          || activeControlInfo.linear.y < -1.01 || activeControlInfo.angular.z > 1.01
  ...
  double rollRad = roll.state * M_PI / 180;
  double pitchRad = pitch.state * M_PI / 180;
  ...
  yaw.predict(tsMillis, varAccelerationError_yaw,
              TooN::makeVector(tsSeconds * yawSpeedControlGain / 2, yawSpeedControlGain), 1, 5 * 5);
  yaw.state[0] = angleFromTo(yaw.state[0], -180, 180);
  ...
  z.predict(
      tsMillis,
      TooN::makeVector(tsSeconds * tsSeconds * tsSeconds * tsSeconds, 9 * tsSeconds,
...
void DroneKalmanFilter::observeIMU_XYZ(const ardrone_autonomy::Navdata* nav)
{
  ...
  double z_obs = (nav->altd * 0.001)
      / sqrt(
          1.0 + (tan(roll.state * 3.14159268 / 180) * tan(roll.state * 3.14159268 / 180))
              + (tan(pitch.state * 3.14159268 / 180) * tan(pitch.state * 3.14159268 / 180)));

  if (!std::isfinite(z_obs))
    z_obs = nav->altd * 0.001;

  if (baselineZ_Filter <= -DBL_MAX)	// only for initialization.
  {
    baselineZ_IMU = z_obs;
  ...
  if (abs(z_obs - baselineZ_IMU) < 0.150)	// jumps of more than 150mm in 40ms are ignored
  {
    ...
    if (lastPosesValid)
    {  // we can't use the z_obs at face value because the sonar might have bounced off something other than the ground
...
void DroneKalmanFilter::updateScaleXYZ(TooN::Vector<3> ptamDiff, TooN::Vector<3> imuDiff, TooN::Vector<3> OrgPtamPose)
{
  ...
  {
    if ((*scalePairs).size() < 5
        || ((*scalePairs)[i].alphaSingleEstimate > median * 0.2 && (*scalePairs)[i].alphaSingleEstimate < median / 0.2))
    {
  ...
  double scale_Unfiltered = (*scalePairs)[0].computeEstimator(sumPP + totSumPP, sumII + totSumII, sumPI + totSumPI, 0.2,
                                                              0.01);
  double scale_PTAMSmallVar = (*scalePairs)[0].computeEstimator(sumPP + totSumPP, sumII + totSumII, sumPI + totSumPI,
                                                                0.00001, 1);
  double scale_IMUSmallVar = (*scalePairs)[0].computeEstimator(sumPP + totSumPP, sumII + totSumII, sumPI + totSumPI, 1,
                                                               0.00001);
  ...
  printf(
      ...
      numIn, numOut, scale_Filtered, scale_PTAMSmallVar, scale_Unfiltered, scale_IMUSmallVar,
      (*scalePairs)[0].computeEstimator(sumPPxy, sumIIxy, sumPIxy, 0.00001, 1), scale_Filtered_xy,
      (*scalePairs)[0].computeEstimator(sumPPxy, sumIIxy, sumPIxy, 1, 0.00001),
      (*scalePairs)[0].computeEstimator(sumPPz, sumIIz, sumPIz, 0.00001, 1), scale_Filtered_z,
...
void DroneKalmanFilter::predictUpTo(int timestamp, bool consume, bool useControlGains)
{
  ...
  while (controlIterator != velQueue->end() && controlIterator + 1 != velQueue->end()
  ...
  while (xyzIterator != navdataQueue->end() && getMS(xyzIterator->header.stamp) <= predictdUpToTimestamp + delayXYZ)
  {
    xyzIterator++;
  }

  std::deque<ardrone_autonomy::Navdata>::iterator rpyIterator = navdataQueue->begin();
  while (rpyIterator != navdataQueue->end() && getMS(rpyIterator->header.stamp) <= predictdUpToTimestamp + delayRPY)
  {
    if (consume)
    {
  ...
  while (true)
  {
    ...
    while (xyzIterator != navdataQueue->end() && getMS(xyzIterator->header.stamp) - delayXYZ < predictdUpToTimestamp)
    {
      xyzIterator++;
    }
    while (rpyIterator != navdataQueue->end() && getMS(rpyIterator->header.stamp) - delayRPY < predictdUpToTimestamp)
    {
      rpyIterator++;
    }
    ...
    if (consume)
    {
      if (node->logfileFilter != NULL)
      {
        ...
        (*(node->logfileFilter)) << predictdUpToTimestamp << " " << 0 << " " << 0 << " " << 0 << " " << 0 << " " << 0
            << " " << 0 << " " << controlIterator->twist.linear.y << " " << controlIterator->twist.linear.x << " "
            << controlIterator->twist.linear.z << " " << controlIterator->twist.angular.z << " "
            << (observedRPY ? rpyIterator->rotX : -1) << " " << (observedRPY ? rpyIterator->rotY : -1) << " "
            << (observedRPY ? lastdYaw : -1) << " " << (observedXYZ ? xyzIterator->vx : -1) << " "
            << (observedXYZ ? xyzIterator->vy : -1) << " " << (observedXYZ ? lastdZ : -1) << " " << x.state[0] << " "
            << y.state[0] << " " << z.state[0] << " " << roll.state << " " << pitch.state << " " << yaw.state[0] << " "
            << x.state[1] << " " << y.state[1] << " " << z.state[1] << " " << yaw.state[1] << " " << lastVXGain << " "
            << lastVYGain << " " << std::endl;
...
{
  return TooN::makeVector(x.state[0], y.state[0], z.state[0], roll.state, pitch.state, yaw.state[0], x.state[1],
                          y.state[1], z.state[1], yaw.state[1]);
}

TooN::Vector<10> DroneKalmanFilter::getCurrentPoseSpeedVariances()
{
  return TooN::makeVector(x.var(0, 0), y.var(0, 0), z.var(0, 0), roll.var, pitch.var, yaw.var(0, 0), x.var(1, 1),
                          y.var(1, 1), z.var(1, 1), yaw.var(1, 1));
}
...
{
  return TooN::makeVector(scale_xyz_initialized ? xy_scale : 1, scale_xyz_initialized ? xy_scale : 1,
                          scale_xyz_initialized ? z_scale : 1);
}
TooN::Vector<3> DroneKalmanFilter::getCurrentScalesForLog()
{
  return TooN::makeVector(scale_xyz_initialized ? scale_from_xy : 1, scale_xyz_initialized ? scale_from_z : 1,
