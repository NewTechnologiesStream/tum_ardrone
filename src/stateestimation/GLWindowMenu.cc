...
void GLWindowMenu::GUICommandHandler(string sCommand, string sParams)
{
  ...
  if (sCommand == msName + ".AddMenuSlider")
  {
    if (vs.size() < 5)
    {
      cout << "? GLWindowMenu.AddMenuSlider: Need 5/6 params: Target Menu, Name, gvar_int name, min, max, NextMenu=\"\""
          << endl;
      return;
    };
    MenuItem m;
  ...
  for (vector<MenuItem>::reverse_iterator i = m.mvItems.rbegin(); i != m.mvItems.rend(); i++)
  {
    switch (i->type)
    {
      case Button:
        ...
        glw.PrintString(ImageRef(nBase + 3, mnMenuTop + *mgvnMenuTextOffset), i->sName);
        break;
      ...
      case Slider:
      {
        ...
        glw.PrintString(ImageRef(nBase + 3, mnMenuTop + *mgvnMenuTextOffset), ost.str());
      }
        break;
    }
