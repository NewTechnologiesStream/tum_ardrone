...
void PTAMWrapper::ResetInternal()
{
  ...
  fleH.close();
  std::cout << "Set Camera Paramerer to: " << camPar[0] << " " << camPar[1] << " " << camPar[2] << " " << camPar[3]
      << " " << camPar[4] << std::endl;
...
void PTAMWrapper::setPTAMPars(double minKFTimeDist, double minKFWiggleDist, double minKFDist, double min_tol,
                              double max_tol)
{
  if (mpMapMaker != 0)
  {
...
void PTAMWrapper::HandleFrame()
{
  ...
  // 2. convert to se3
  predConvert->setPosRPY(PTAMPoseGuess[0], PTAMPoseGuess[1], PTAMPoseGuess[2], PTAMPoseGuess[3], PTAMPoseGuess[4],
                         PTAMPoseGuess[5]);
  ...
  TooN::Vector < 6 > PTAMResult = TooN::makeVector(predConvert->x, predConvert->y, predConvert->z, predConvert->roll,
  ...
  if (mpTracker->lastStepResult == mpTracker->I_SECOND)
  {
    ...
    filter->setCurrentScales(
        TooN::makeVector(mpMapMaker->initialScaleFactor * 1.2, mpMapMaker->initialScaleFactor * 1.2,
  ...
  if (isGoodCount >= 3)
  {
    ...
    if (includedTime >= 2000 && framesIncludedForScaleXYZ > 1)	// ADD! (if too many, was resetted before...)
    {
      ...
      TooN::Vector < 3 > diffIMU = evalNavQue(
          ptamPositionForScaleTakenTimestamp - filter->delayVideo + filter->delayXYZ,
          mimFrameTime_workingCopy - filter->delayVideo + filter->delayXYZ, &zCorrupted, &allCorrupted, &pressureStart,
          &pressureEnd);
  ...
  if (lockNextFrame && isGood)
  {
    ...
    snprintf(charBuf, 500, "locking scale fixpoint to %.3f %.3f %.3f", PTAMResultTransformed[0],
             PTAMResultTransformed[1], PTAMResultTransformed[2]);
    ROS_INFO("%s", charBuf);
    node->publishCommand(std::string("u l ") + charBuf);
  }
  ...
  if (!mapLocked && rand() % 5 == 0)
  {
    ...
    for (unsigned int i = 0; i < mpMap->vpKeyFrames.size(); i++)
    {
      ...
      TooN::Vector < 6 > CamPos = TooN::makeVector(predConvert->x, predConvert->y, predConvert->z, predConvert->roll,
                                                   predConvert->pitch, predConvert->yaw);
    ...
    if (flushMapKeypoints)
    {
      ...
      {
        (*fle) << mapPointsTransformed[i][0] << " " << mapPointsTransformed[i][1] << " " << mapPointsTransformed[i][2]
            << std::endl;
      }
  ...
  {
    // render grid
    predConvert->setPosRPY(filterPosePostPTAM[0], filterPosePostPTAM[1], filterPosePostPTAM[2], filterPosePostPTAM[3],
  ...
  if (node->logfilePTAM != NULL)
  {
    ...
    if (node->logfilePTAM != NULL)
    {
      (*(node->logfilePTAM)) << (isGood ? (isVeryGood ? 2 : 1) : 0) << " "
          ...
          << filterPosePrePTAM[4] << " " << filterPosePrePTAM[5] << " " << filterPosePrePTAM[6] << " "
          << filterPosePrePTAM[7] << " " << filterPosePrePTAM[8] << " " << filterPosePrePTAM[9] << " "
          ...
          << filterPosePostPTAM[9] << " " << PTAMResultTransformed[0] << " " << PTAMResultTransformed[1] << " "
          << PTAMResultTransformed[2] << " " << PTAMResultTransformed[3] << " " << PTAMResultTransformed[4] << " "
          << PTAMResultTransformed[5] << " " << scales[0] << " " << scales[1] << " " << scales[2] << " " << offsets[0]
          << " " << offsets[1] << " " << offsets[2] << " " << offsets[3] << " " << offsets[4] << " " << offsets[5]
          << " " << sums[0] << " " << sums[1] << " " << sums[2] << " " << PTAMResult[0] << " " << PTAMResult[1] << " "
          << PTAMResult[2] << " " << PTAMResult[3] << " " << PTAMResult[4] << " " << PTAMResult[5] << " "
...
TooN::Vector<3> PTAMWrapper::evalNavQue(unsigned int from, unsigned int to, bool* zCorrupted, bool* allCorrupted,
                                        float* out_start_pressure, float* out_end_pressure)
{
  ...
  for (std::deque<ardrone_autonomy::Navdata>::iterator cur = navInfoQueue.begin(); cur != navInfoQueue.end();)
  {
  ...
  for (std::deque<ardrone_autonomy::Navdata>::iterator cur = navInfoQueue.begin(); cur != navInfoQueue.end(); cur++)
  {
